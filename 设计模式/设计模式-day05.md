...
public class EndState implements State {
    ...
    public void doAction(Context context) {
    	...
    	//这里可以看出，每次对象状态发生改变时，这个上下文对象的内部状态也发生改变，
        context.setState(this);
    }
...
* **因为Colleague之间的交流是通过mediator进行的，所以他们要聚合mediator，又因为ConcreteMediator要给Colleagues发送通知，所以他们必须聚合每个colleague，并且要实现联系（contact）方法。**
...
public class GameRole {
    ...
    //保存角色状态，将当前对象的属性赋给它的内部对象Memento
    public RoleStateMemento saveState() {
        return new RoleStateMemento(vit, atk, def);
    }

    //回复角色状态，将Memento记录的属性复制给当前对象
