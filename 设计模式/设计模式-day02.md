...
<img src="img/简单工厂模式.png" style="zoom: 200%;" />
...
​	stu对象和stu1对象是同一个对象，就会产生将stu1对象中name属性值改为“李四”，两个Citation（奖状）对象中显示的都是李四。这就是浅克隆的效果，对具体原型类（Citation）中的引用类型的属性进行**引用的复制**。这种情况需要使用深克隆，而进行深克隆需要使用对象流。代码如下：
...
**浅克隆 ： 将对象的每个属性进行依次复制，但当对象的属性是引用类型时，实质上复制的是其引用，当一个克隆出来的对象改变了其引用指向的值时，另一个克隆出来的对象的该引用数据类型所指向的值也会随之改变**

**深克隆：深克隆对于引用数据，递归至基本类型后再复制。深拷贝后的对象与原来的对象是完全隔离的，互不影响，对一个对象的修改并不会印象另一对象。**
